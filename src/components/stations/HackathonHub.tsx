  ...
  useGSAP(
    () => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        gsap.utils.toArray<HTMLElement>(".hack-card").forEach((card, i) => {
          gsap.from(card, {
            y: 50,
            opacity: 0,
            scale: 0.95,
        ...
        gsap.utils.toArray<HTMLElement>(".hack-pill").forEach((pill, i) => {
          gsap.from(pill, {
            ...
            ease: "back.out(2)",
            scrollTrigger: {
              trigger: pill.closest(".hack-card"),
              start: "top 88%",
            },
          });
        });
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {hackathons.map((hack, i) => (
            <article
              key={i}
              className="hack-card group relative overflow-hidden rounded-lg transition-transform duration-300 hover:scale-[1.02]"
              style={{
